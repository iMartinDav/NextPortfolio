...
export async function GET(): Promise<Response> {
  ...
  try {
    ...
    while (nextUrl) {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          return new Response(
            JSON.stringify({ error: "Failed to parse error response", totalStars: 0 }),
            { 
              status: 200,
              headers: { 'Content-Type': 'application/json' } 
            }
        ...
        const errorMessage = errorHandler[statusCode as keyof typeof errorHandler] || errorHandler.default;
      ...
      totalStars = repos.reduce((sum, repo) => sum + repo.stargazers_count, totalStars);
    ...
    return new Response(
      JSON.stringify({ totalStars, mockData: false }),
      { 
        status: 200, 
        headers: { 'Content-Type': 'application/json' }
      }
